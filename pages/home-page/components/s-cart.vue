			...
			<block v-for="(item,index) in shopping_card" :key="index">
				<view class="goods-list" v-if="item.quantity >0">
					...
					<view class="goods-quantity">
						<view><image src="/static/tab/jianhao.png" mode="widthFix" @click="reduce(index,item.quantity,item.cid,item._id,item.goodsindex,item.unitprice)"></image></view>
						<view>{{item.quantity}}</view>
						<view><image src="/static/tab/jia.png" mode="widthFix" @click="add(index,item.quantity,item.cid,item._id,item.goodsindex,item.unitprice)"></image></view>
					</view>
				</view>
			</block>
...
export default {
	...
	methods:{
		...
		// 减少
		reduce(index,quantity,cid,_id,goodsindex,unitprice){
			const QU = quantity -1 
			this.$parent.shopping_Cart_add_sub(index,QU,cid,_id,goodsindex,unitprice)
		},
		
		// 增加
		add(index,quantity,cid,_id,goodsindex,unitprice){
			const QU = quantity +1 
			this.$parent.shopping_Cart_add_sub(index,QU,cid,_id,goodsindex,unitprice)
		},
